...
static void clearState(unsigned char* state) { memcpy((void*)state, (const void*)&stateDefault, STATE_BUF_LEN); }
...
void hash(char output[STATE_BUF_LEN], const char* input, const int length) {
    ...
    for (uint8_t p = 0; p < PASSES; p++) {
        ...
        for (uint32_t i = 0; i < newLength; i++) {
            ...
            // The below LONG array access of paddedInput is a replica of the RING_BUF_INDEX function but with loop around the length of
            // paddedInput, instead of the state buffer.
            state[RING_BUF_INDEX(i)] +=
                (cbyte ^ (lbyte)) +
                paddedInput[(signed int)(i) % newLength < 0 ? (signed int)(i) % newLength + newLength : (signed int)(i) % newLength];

            // Below lines are INSPIRED by MD6's compression function as listed in the MIT paper:
            ...
            // state[RING_BUF_INDEX(i)] ^= ((state[RING_BUF_INDEX(i)] >> COMPRESSION_SHIFTS[(uint8_t)((cbyte) & 0xF % 5)])) |
            // (state[RING_BUF_INDEX(i)] << (8 - COMPRESSION_SHIFTS[(uint8_t)((cbyte) & 0xF % 5)])); Grab least significantt 4 bits from
            // cbyte, then modulo 5 to index the shift array (no zero shifts!). XOR the current state byte with the shifted state byte to
            // add more pseudo-randomness.
            lbyte = state[RING_BUF_INDEX(i)]; // Pushes back the first found collision by 10,000. But it is still at 3 characters. 4
                                              // characters is the next milestone!
        }
    }
