...
// Quick string hash functions from the below SO link (only slightly modified). Used for (slightly slower) switch cases with strings because it's fun
    ...
    for(std::cout << "Command> "; std::cin >> line && line != "exit" && line != "quit" && line != "q"; std::cout << "Command> ") {
        switch(hash(line)){
            ...
            case "l"_:
            {
                ...
                try{
                    /* The monadic operations for std::optional are only available in the experimental C++23, so I will just use value_or() for now. */
